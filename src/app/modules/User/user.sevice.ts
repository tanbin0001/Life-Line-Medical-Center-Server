...
const createAdmin = async (req: Request): Promise<Admin> => {
    ...
    if (file) {
        ...
        req.body.admin.profilePhoto = uploadFileToCloudinary?.secure_url
    }
...
const createDoctor = async (req: Request): Promise<Doctor> => {
    ...
    if (file) {
        const uploadFileToCloudinary= await  uploadToCloudinary(file);
        req.body.doctor.profilePhoto = uploadFileToCloudinary?.secure_url
    }
...
const updateMyProfie = async (user: IAuthUser, req: Request) => {
    ...
    if (file) {
        const uploadFileToCloudinary = await uploadToCloudinary(file);
        req.body.profilePhoto = uploadFileToCloudinary?.secure_url;
    }
